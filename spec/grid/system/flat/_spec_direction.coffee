describe "direction", ->
  ...
  it "doit pointer vers le nord", ->
    ...
    # then
    expected = priority: ( require "./side" ).NORTH , x: 0, y: +1, z: -1
  ...
  it "doit pointer vers le nord-est", ->
    ...
    # then
    expected = priority: ( require "./side" ).NORTH_EAST , x: +1, y: 0, z: -1
  ...
  it "doit pointer vers le nord-ouest", ->
    ...
    # then
    expected = priority: ( require "./side" ).NORTH_WEST , x: -1, y: +1, z: 0
  ...
  it "doit pointer vers le sud", ->
    ...
    # then
    expected = priority: ( require "./side" ).SOUTH , x: 0, y: -1, z: +1
  ...
  it "doit pointer vers le sud-est", ->
    ...
    # then
    expected = priority: ( require "./side" ).SOUTH_EAST , x: +1, y: -1, z: 0
    expect( JSON.stringify southEast ).toBe( JSON.stringify expected )

  it "doit pointer vers le sud-ouest", ->
    southWest = direction.southWest
    # then
    expected = priority: ( require "./side" ).SOUTH_WEST , x: -1, y: 0, z: +1
